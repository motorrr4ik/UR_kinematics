...
def inverse_kinematics(config_matrix, transformation_mat):
    ...
    phi = np.arccos(config_matrix[3,2]/np.sqrt(P_05[0]**2 + P_05[1]**2))
    ...
    #theta 5
    for i in {0, 4}:
            cos_theta5 = (T[0, 3] * np.sin(theta[0, i]) - T[1, 3] * np.cos(theta[0, i]) - 
    ...
    for i in {0, 2, 4, 6}:
        ...
        if np.sin(theta[4,i]) == 0:
            ...
            theta6 = np.arctan2((-T_inv[1, 0] * np.sin(theta[0, i]) + T_inv[1, 1] * np.cos(theta[0, i]))/np.sin(theta[4,i]),
                    (T_inv[0, 0] * np.sin(theta[0, i]) - T_inv[0, 1] * np.cos(theta[0, i]))/np.sin(theta[4,i]))
    ...
    for i in range(8):
        ...
        theta[1, i] = atan2(-P13[1], -P13[0]) - asin(
            -config_matrix[2, 0] * np.sin(theta[2, i]) / np.sqrt(P13[0] ** 2 + P13[1] ** 2)
        )
        ...
        theta[3, i] = np.arctan2(T34[1, 0], T34[0, 0]) 
    return theta